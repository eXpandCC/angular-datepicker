...
test.describe("dpDayPicker dayPicker", () => {
  ...
  test("should check if go to current location btn is working as expected", async () => {
    ...
    const commonDayCalendar = async (menu: Locator, input: Locator) => {
      ...
      await expect(await po.currentLocationBtn()).not.toBeDisabled(); // El botón no debe estar deshabilitado cuando está activo
      ...
      await input.click();
      await expect(await po.currentLocationBtn()).toBeVisible(); // El botón sigue visible
      await expect(await po.currentLocationBtn()).toBeDisabled(); // El botón debe estar deshabilitado cuando no se debe usar
      await po.dayCalendarNavHeaderBtn().click();
      await expect(await po.currentLocationBtn()).toBeVisible();
      await expect(await po.currentLocationBtn()).toBeDisabled(); // Verifica que el botón sigue deshabilitado
    };
    ...
    const commonMonth = async (menu: Locator, input: Locator) => {
      ...
      await input.click();
      await expect(await po.currentLocationBtn()).toBeVisible();
      await expect(await po.currentLocationBtn()).not.toBeDisabled();
      ...
      await input.click();
      await expect(await po.currentLocationBtn()).toBeVisible();
      await expect(await po.currentLocationBtn()).toBeDisabled();
    };
